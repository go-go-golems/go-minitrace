...
bash_calls AS (
  SELECT
    ...
    REPLACE(CAST(json_extract(tc, '$.timestamp') AS VARCHAR), '"', '') AS ts,
...
SELECT run, tool_call_id, ts, SUBSTR(command, 1, 140) AS command_snippet, SUBSTR(result, 1, 220) AS result_snippet
FROM bash_calls
WHERE LOWER(result) LIKE '%fail%'
   OR LOWER(result) LIKE '%error%'
   OR LOWER(result) LIKE '%missing cases%'
   OR LOWER(result) LIKE '%no such file%'
   OR LOWER(result) LIKE '%file does not exist%'
   OR LOWER(result) LIKE '%exited with code%'
ORDER BY run, ts;
