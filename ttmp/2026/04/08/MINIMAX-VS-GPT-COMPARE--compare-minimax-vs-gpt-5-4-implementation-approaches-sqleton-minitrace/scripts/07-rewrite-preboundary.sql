...
rewrites AS (
  ...
  WHERE REPLACE(CAST(json_extract(tc, '$.timestamp') AS VARCHAR), '"', '') BETWEEN b.start_ts AND b.boundary_ts
    AND REPLACE(CAST(json_extract(tc, '$.tool_name') AS VARCHAR), '"', '') IN ('write', 'edit')
    AND json_extract(tc, '$.input.file_path') IS NOT NULL
)
SELECT run, file_path, tool_name, COUNT(*) AS touches, MIN(ts) AS first_touch, MAX(ts) AS last_touch
FROM rewrites
GROUP BY run, file_path, tool_name
HAVING COUNT(*) > 1
ORDER BY run, touches DESC, file_path, tool_name;
