...
- [ ] Remove the legacy session route registrations from `cmd/go-minitrace/cmds/serve/server.go`
- [ ] Remove the unused v1 session HTTP handlers while keeping the shared normalization/helper code needed by v2
- [ ] Replace direct v1 session handler tests with route-level coverage that proves those legacy endpoints now return 404 while v2 handlers remain covered
- [ ] Remove the legacy preset/query route registrations from `cmd/go-minitrace/cmds/serve/server.go`
- [ ] Remove the unused v1 preset/query handlers while keeping the shared SQL file helper/types used by v2
- [ ] Replace direct v1 preset/query tests with route-level coverage for removed endpoints and keep v2 CRUD coverage intact
