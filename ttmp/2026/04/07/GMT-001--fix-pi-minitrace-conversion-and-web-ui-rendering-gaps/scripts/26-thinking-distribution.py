#!/usr/bin/env python3
"""Analyze thinking block distribution across a minitrace session.
Usage: python3 26-thinking-distribution.py [minitrace.json]
"""
import json, sys
...
print(f"With thinking:     {len(with_thinking)}  ({100*len(with_thinking)//max(len(assistant),1)}%)")
print()
