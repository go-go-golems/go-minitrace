...
- Fixed API-side default hydration for direct `/api/v2/query-commands/.../execute` requests by routing resolved command values through Glazed parsing/default middleware before SQL render or JS execution, and added serve tests for SQL render-only, direct SQL execution, direct JS execution with omitted defaults, and explicit precedence order (caller override > alias defaults > command defaults)
